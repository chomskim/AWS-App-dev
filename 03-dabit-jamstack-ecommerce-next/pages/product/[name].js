...
import { slugify, formatPrice } from '../../utils/helpers'
    ...
    <>
      ...
      >
        ...
        <div className='pt-2 px-0 md:px-10 pb-8 w-full md:w-1/2'>
          ...
          <h2 className='text-2xl tracking-wide sm:py-8 py-6'>{formatPrice(price)}</h2>
          <p className='text-gray-600 leading-7'>{description}</p>
          <div className='my-6'>
            <QuantityPicker increment={increment} decrement={decrement} numberOfitems={numberOfitems} />
          </div>
          <Button full title='Add to Cart' onClick={() => addItemToCart(product)} />
        </div>
      </div>
    </>
  )
...
function ItemViewWithContext(props) {
  return (
    <ContextProviderComponent>
      <SiteContext.Consumer>{(context) => <ItemView {...props} context={context} />}</SiteContext.Consumer>
    </ContextProviderComponent>
  )
}
